...
class NLTKProcessor(NaturalLanguageProcessor):
    def process(self):
        try:
            ...
            print('WARNING: Falling back to SimpleProcessor: ', e, file=sys.stderr)
    ...
    def processWithNLTK(self):
        ...
        self.pos_tags = [x[1] for x in nltk.pos_tag(self.tokens)]
...
def sortRules(rules):
    ...
    while 'white' in list(colors.values()):
...
def parseUtterance(utterance, rules, verbose=0):
    ...
    def applyRule(start, end, mid, rule, rhsIndex, children, score):
        ...
        print('apply rule', rhsIndex, len(rule.rhs), rule.rhs[rhsIndex])
        ...
        if isCat(a):  # Category
            ...
            for split in range(mid+1, end+1):
                print("split", mid, split, chart[mid][split].get(a, {}))
                for child in chart[mid][split].get(a, {}):
                    print(child, rhsIndex+1)
